...
fn expand_around_center(
    ...
    while left_index >= 0 && right_index < length {
        // Worst case complexity: O(N) with constant space

        let left_char = &string_vec[left_index as usize];
        let right_char = &string_vec[right_index];
        if left_char != right_char {
            // Note: Rust doesn't compare by reference so this is considered safe unlike Java for example
